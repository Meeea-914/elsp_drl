...
class HookSubprocVecEnv(Hook, SubprocVecEnv):
    hook_class = SubprocVecEnv
    hook_methods = ['step_wait']
...
def evaluate(env: SubprocVecEnv, eval_times, deterministic, model):
    ...
    while True:
        ...
        for i in range(len(obs_list)):
            actions.append(int(model.predict(obs_list[i], deterministic=deterministic)[0][0]))
...
def evaluate_ssa(env_no, demand_scale, model_path, result_xlsx_path,):
    ...
    eval_data_saver.algorithm = 'PPO_SSA2_{}_{}'.format(cpu_num, 'T' if deterministic else 'F')
