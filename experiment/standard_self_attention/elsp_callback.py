...
from experiment.standard_self_attention.evaluate_elsp import evaluate
...
class CustomCallback(BaseCallback):
    ...
    def __init__(self, _env: SubprocVecEnv, _model: PPO, verbose=0, eval_th=32.5e4, n_eval_steps=15, no_eval=False):
        ...
        self.path = './result/env_path_%s/algorithm_%s/total_%e_n_steps_%e_batch_size_%e_n_epochs_%e_env_num_%d_lr_%e_gama_%.3f/time_%s' % \
                    (self.model.env_path, self.model.__class__.__name__+'+SSA', self.model.total_steps, self.model.n_steps, self.model.batch_size,
    ...
    def _on_rollout_end(self) -> None:
        ...
        if False and self.mean_profits[-1] > self.eval_th:
            self.eval_cnt += 1
            print(self.eval_cnt)
    ...
    def _on_training_end(self) -> None:
        ...
        for i in range(1, len(self.mean_action_num)):
            plt.plot(x, self.mean_action_num[i], color=c[i%len(c)], label=('$product:%d$' % i) if i > 0 else '$idle$')
        plt.legend()
